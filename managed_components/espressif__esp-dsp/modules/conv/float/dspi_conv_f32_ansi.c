...
static inline float conv_block(const float *i_data, int i_pos_x, const image2d_t *in_image, const image2d_t *filter,
                               int m_start, int m_end, int n_start, int n_end, int i_step, int f_step)
{
    float acc = 0;
    const float *f_data = (const float *)filter->data + m_start * f_step;
...
esp_err_t dspi_conv_f32_ansi(const image2d_t *in_image, const image2d_t *filter, image2d_t *out_image)
{
    ...
    for (int y = 0 ; y < rest_y; y++ ) {
        ...
        for (int x = 0 ; x < rest_x; x++) {
            int i_pos_x = i_pos_y;
            int m_start = rest_y - y;
            int m_end = filter->size_y;
            int n_start = rest_x - x;
            int n_end = filter->size_x;
            float acc = conv_block(i_data, i_pos_x, in_image, filter, m_start, m_end, n_start, n_end, i_step, f_step);
        ...
        for (int x = in_image->size_x - filter->size_x / 2 - 1; x < in_image->size_x; x++) {
            ...
            int m_end = filter->size_y;
            int n_start = 0;
            int n_end = filter->size_x - (x - in_image->size_x + filter->size_x / 2 + 1);
            float acc = conv_block(i_data, i_pos_x, in_image, filter, m_start, m_end, n_start, n_end, i_step, f_step);
            i_pos_y += in_image->step_x;
            out_data[x * out_image->step_x + y * out_image->stride_x * out_image->step_y] = acc;
        }
    ...
    for (int y = in_image->size_y - filter->size_y / 2 ; y < in_image->size_y; y++ ) {
        ...
        for (int x = 0 ; x < rest_x; x++) {
            int i_pos_x = i_pos_y;
            int m_start = 0;
            int m_end = filter->size_y - (y - in_image->size_y + filter->size_y / 2 + 1);
            int n_start = rest_x - x;
            int n_end = filter->size_x;
            float acc = conv_block(i_data, i_pos_x, in_image, filter, m_start, m_end, n_start, n_end, i_step, f_step);
            i_pos_y += in_image->step_x;
